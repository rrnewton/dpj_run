...
/**
 * The {@link DPJPartition} class, specialized to {@code double}.
 *
 * <p>The {@code DPJPartitionDouble} class represents an array of {@link
 * DPJArrayDouble} objects that partition another {@code DPJArrayDouble} (called
 * the <i>root array</i>).  Each {@code DPJArrayDouble} in the partition is
 * a contiguous subsection of the root array, and all are mutually
 * disjoint.  The {@code DPJArrayDouble}s in the partition are called
 ...
 */
public class DPJPartitionDouble<region R> {

    /**
     * The array being partitioned
     */
    private final DPJArrayDouble<R> A;

    /**
     * The segments of the partition
     */
    private final DPJArrayDouble<this:[i]>[]<this:[i]>#i segs;
     ...
     */
    public DPJPartitionDouble(DPJArrayDouble<R> A, final int idx) 
	writes this:[?] {
	this.A = A;
	this.length = 2;
	this.stride = 0;
	segs = (DPJArrayDouble<this:[i]>[]<this:[i]>#i) new DPJArrayDouble[length]<this:[i]>#i;
	segs[0] = (DPJArrayDouble<this:[0]>) A.subarray(0, idx);
	segs[1] = (DPJArrayDouble<this:[1]>) A.subarray(idx, A.length - idx);
    }
     ...
     */
    public DPJPartitionDouble(DPJArrayDouble<R> A, final int idx, boolean exclude) 
	writes this:[?] {
	this.A = A;
	this.length = 2;
	this.stride = 0;
	this.segs = (DPJArrayDouble<this:[i]>[]<this:[i]>#i) new DPJArrayDouble[length]<this:[i]>#i;
	segs[0] = (DPJArrayDouble<this:[0]>) A.subarray(0, idx);
	if (exclude) {
	    segs[1] = (DPJArrayDouble<this:[1]>) A.subarray(idx + 1, A.length - idx - 1);
	} else {
            segs[1] = (DPJArrayDouble<this:[1]>) A.subarray(idx, A.length - idx);
	}
    }
     ...
     */
    private DPJPartitionDouble(DPJArrayDouble<R> A, int stride, double strided) pure {
    	this.A = A;
        this.stride = stride;
     ...
     */
    public static <region R>DPJPartitionDouble<R> 
      stridedPartition(DPJArrayDouble<R> A, int stride) pure {
    	return new DPJPartitionDouble<R>(A, stride, 0.0);
    }
     ...
     * split points.
     */
    public <region RI>DPJPartitionDouble(DPJArrayDouble<R> A, int[]<RI> idxs) 
	reads RI writes this:[?] {
	this.A = A;
	this.length = idxs.length+1;
	this.segs = (DPJArrayDouble<this:[i]>[]<this:[i]>#i) 
	    new DPJArrayDouble[length]<this:[i]>#i;
	this.stride = 0;
	if (length == 1)
	    segs[0] = (DPJArrayDouble<this:[0]>) A;
	else {
	    int i = 0, len = 0;
	    segs[0] = (DPJArrayDouble<this:[0]>) A.subarray(0, idxs[0]);
	    for (i = 1; i < idxs.length; ++i) {
		len = idxs[i] - idxs[i-1];
		if (len < 0) 
		    throw new ArrayIndexOutOfBoundsException();
	    	final int j = i;
		segs[j] = (DPJArrayDouble<this:[j]>) A.subarray(idxs[j-1], len);	    
	    }
            i = idxs[idxs.length - 1];
            len = A.length - i;
	    final int length = idxs.length;
            segs[length] = (DPJArrayDouble<this:[length]>) A.subarray(i, len);
	}
    }
     ...
     */
    public DPJArrayDouble<this : [idx] : *> get(final int idx) reads this:[idx] {
	...
	if (segs != null)
	    ...
	    return (DPJArrayDouble<this:[idx]:*>) A.subarray(start, segLength);
        }
    }
