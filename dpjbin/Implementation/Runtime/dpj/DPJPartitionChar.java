...
/**
 * The {@link DPJPartition} class, specialized to {@code char}.
 *
 * <p>The {@code DPJPartitionChar} class represents an array of {@link
 * DPJArrayChar} objects that partition another {@code DPJArrayChar} (called
 * the <i>root array</i>).  Each {@code DPJArrayChar} in the partition is
 * a contiguous subsection of the root array, and all are mutually
 * disjoint.  The {@code DPJArrayChar}s in the partition are called
 ...
 */
public class DPJPartitionChar<region R> {

    /**
     * The array being partitioned
     */
    private final DPJArrayChar<R> A;

    /**
     * The segments of the partition
     */
    private final DPJArrayChar<this:[i]>[]<this:[i]>#i segs;
     ...
     */
    public DPJPartitionChar(DPJArrayChar<R> A, final int idx) 
	writes this:[?] {
	this.A = A;
	this.length = 2;
	this.stride = 0;
	segs = (DPJArrayChar<this:[i]>[]<this:[i]>#i) new DPJArrayChar[length]<this:[i]>#i;
	segs[0] = (DPJArrayChar<this:[0]>) A.subarray(0, idx);
	segs[1] = (DPJArrayChar<this:[1]>) A.subarray(idx, A.length - idx);
    }
     ...
     */
    public DPJPartitionChar(DPJArrayChar<R> A, final int idx, boolean exclude) 
	writes this:[?] {
	this.A = A;
	this.length = 2;
	this.stride = 0;
	this.segs = (DPJArrayChar<this:[i]>[]<this:[i]>#i) new DPJArrayChar[length]<this:[i]>#i;
	segs[0] = (DPJArrayChar<this:[0]>) A.subarray(0, idx);
	if (exclude) {
	    segs[1] = (DPJArrayChar<this:[1]>) A.subarray(idx + 1, A.length - idx - 1);
	} else {
            segs[1] = (DPJArrayChar<this:[1]>) A.subarray(idx, A.length - idx);
	}
    }
     ...
     */
    private DPJPartitionChar(DPJArrayChar<R> A, int stride, double strided) pure {
    	this.A = A;
        this.stride = stride;
     ...
     */
    public static <region R>DPJPartitionChar<R> 
      stridedPartition(DPJArrayChar<R> A, int stride) pure {
    	return new DPJPartitionChar<R>(A, stride, 0.0);
    }
     ...
     * split points.
     */
    public <region RI>DPJPartitionChar(DPJArrayChar<R> A, int[]<RI> idxs) 
	reads RI writes this:[?] {
	this.A = A;
	this.length = idxs.length+1;
	this.segs = (DPJArrayChar<this:[i]>[]<this:[i]>#i) 
	    new DPJArrayChar[length]<this:[i]>#i;
	this.stride = 0;
	if (length == 1)
	    segs[0] = (DPJArrayChar<this:[0]>) A;
	else {
	    int i = 0, len = 0;
	    segs[0] = (DPJArrayChar<this:[0]>) A.subarray(0, idxs[0]);
	    for (i = 1; i < idxs.length; ++i) {
		len = idxs[i] - idxs[i-1];
		if (len < 0) 
		    throw new ArrayIndexOutOfBoundsException();
	    	final int j = i;
		segs[j] = (DPJArrayChar<this:[j]>) A.subarray(idxs[j-1], len);	    
	    }
            i = idxs[idxs.length - 1];
            len = A.length - i;
	    final int length = idxs.length;
            segs[length] = (DPJArrayChar<this:[length]>) A.subarray(i, len);
	}
    }
     ...
     */
    public DPJArrayChar<this : [idx] : *> get(final int idx) reads this:[idx] {
	...
	if (segs != null)
	    ...
	    return (DPJArrayChar<this:[idx]:*>) A.subarray(start, segLength);
        }
    }
