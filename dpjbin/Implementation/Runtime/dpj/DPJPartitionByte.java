...
/**
 * The {@link DPJPartition} class, specialized to {@code byte}.
 *
 * <p>The {@code DPJPartitionByte} class represents an array of {@link
 * DPJArrayByte} objects that partition another {@code DPJArrayByte} (called
 * the <i>root array</i>).  Each {@code DPJArrayByte} in the partition is
 * a contiguous subsection of the root array, and all are mutually
 * disjoint.  The {@code DPJArrayByte}s in the partition are called
...
public class DPJPartitionByte<region R> {
     ...
     */
    private final DPJArrayByte<R> A;

    /**
     * The segments of the partition
     */
    private final DPJArrayByte<this:[i]>[]<this:[i]>#i segs;
     ...
     */
    public DPJPartitionByte(DPJArrayByte<R> A, final int idx) 
	writes this:[?] {
	this.A = A;
	this.length = 2;
	this.stride = 0;
	segs = (DPJArrayByte<this:[i]>[]<this:[i]>#i) new DPJArrayByte[length]<this:[i]>#i;
	segs[0] = (DPJArrayByte<this:[0]>) A.subarray(0, idx);
	segs[1] = (DPJArrayByte<this:[1]>) A.subarray(idx, A.length - idx);
    }
     ...
     */
    public DPJPartitionByte(DPJArrayByte<R> A, final int idx, boolean exclude) 
	writes this:[?] {
	this.A = A;
	this.length = 2;
	this.stride = 0;
	this.segs = (DPJArrayByte<this:[i]>[]<this:[i]>#i) new DPJArrayByte[length]<this:[i]>#i;
	segs[0] = (DPJArrayByte<this:[0]>) A.subarray(0, idx);
	if (exclude) {
	    segs[1] = (DPJArrayByte<this:[1]>) A.subarray(idx + 1, A.length - idx - 1);
	} else {
            segs[1] = (DPJArrayByte<this:[1]>) A.subarray(idx, A.length - idx);
	}
    }
     ...
     */
    private DPJPartitionByte(DPJArrayByte<R> A, int stride, double strided) pure {
    	this.A = A;
        this.stride = stride;
     ...
     */
    public static <region R>DPJPartitionByte<R> 
      stridedPartition(DPJArrayByte<R> A, int stride) pure {
    	return new DPJPartitionByte<R>(A, stride, 0.0);
    }
     ...
     * split points.
     */
    public <region RI>DPJPartitionByte(DPJArrayByte<R> A, int[]<RI> idxs) 
	reads RI writes this:[?] {
	this.A = A;
	this.length = idxs.length+1;
	this.segs = (DPJArrayByte<this:[i]>[]<this:[i]>#i) 
	    new DPJArrayByte[length]<this:[i]>#i;
	this.stride = 0;
	if (length == 1)
	    segs[0] = (DPJArrayByte<this:[0]>) A;
	else {
	    int i = 0, len = 0;
	    segs[0] = (DPJArrayByte<this:[0]>) A.subarray(0, idxs[0]);
	    for (i = 1; i < idxs.length; ++i) {
		len = idxs[i] - idxs[i-1];
		if (len < 0) 
		    throw new ArrayIndexOutOfBoundsException();
	    	final int j = i;
		segs[j] = (DPJArrayByte<this:[j]>) A.subarray(idxs[j-1], len);	    
	    }
            i = idxs[idxs.length - 1];
            len = A.length - i;
	    final int length = idxs.length;
            segs[length] = (DPJArrayByte<this:[length]>) A.subarray(i, len);
	}
    }
     ...
     */
    public DPJArrayByte<this : [idx] : *> get(final int idx) reads this:[idx] {
	...
	if (segs != null)
	    ...
	    return (DPJArrayByte<this:[idx]:*>) A.subarray(start, segLength);
        }
    }
