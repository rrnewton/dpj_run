...
/**
 * The {@link DPJPartition} class, specialized to {@code boolean}.
 *
 * <p>The {@code DPJPartitionBoolean} class represents an array of {@link
 * DPJArrayBoolean} objects that partition another {@code DPJArrayBoolean} (called
 * the <i>root array</i>).  Each {@code DPJArrayBoolean} in the partition is
 * a contiguous subsection of the root array, and all are mutually
 * disjoint.  The {@code DPJArrayBoolean}s in the partition are called
...
public class DPJPartitionBoolean<region R> {
     ...
     */
    private final DPJArrayBoolean<R> A;

    /**
     * The segments of the partition
     */
    private final DPJArrayBoolean<this:[i]>[]<this:[i]>#i segs;
     ...
     */
    public DPJPartitionBoolean(DPJArrayBoolean<R> A, final int idx) 
	writes this:[?] {
	this.A = A;
	this.length = 2;
	this.stride = 0;
	segs = (DPJArrayBoolean<this:[i]>[]<this:[i]>#i) new DPJArrayBoolean[length]<this:[i]>#i;
	segs[0] = (DPJArrayBoolean<this:[0]>) A.subarray(0, idx);
	segs[1] = (DPJArrayBoolean<this:[1]>) A.subarray(idx, A.length - idx);
    }
     ...
     */
    public DPJPartitionBoolean(DPJArrayBoolean<R> A, final int idx, boolean exclude) 
	writes this:[?] {
	this.A = A;
	this.length = 2;
	this.stride = 0;
	this.segs = (DPJArrayBoolean<this:[i]>[]<this:[i]>#i) new DPJArrayBoolean[length]<this:[i]>#i;
	segs[0] = (DPJArrayBoolean<this:[0]>) A.subarray(0, idx);
	if (exclude) {
	    segs[1] = (DPJArrayBoolean<this:[1]>) A.subarray(idx + 1, A.length - idx - 1);
	} else {
            segs[1] = (DPJArrayBoolean<this:[1]>) A.subarray(idx, A.length - idx);
	}
    }
     ...
     */
    private DPJPartitionBoolean(DPJArrayBoolean<R> A, int stride, double strided) pure {
    	this.A = A;
        this.stride = stride;
     ...
     */
    public static <region R>DPJPartitionBoolean<R> 
      stridedPartition(DPJArrayBoolean<R> A, int stride) pure {
    	return new DPJPartitionBoolean<R>(A, stride, 0.0);
    }
     ...
     * split points.
     */
    public <region RI>DPJPartitionBoolean(DPJArrayBoolean<R> A, int[]<RI> idxs) 
	reads RI writes this:[?] {
	this.A = A;
	this.length = idxs.length+1;
	this.segs = (DPJArrayBoolean<this:[i]>[]<this:[i]>#i) 
	    new DPJArrayBoolean[length]<this:[i]>#i;
	this.stride = 0;
	if (length == 1)
	    segs[0] = (DPJArrayBoolean<this:[0]>) A;
	else {
	    int i = 0, len = 0;
	    segs[0] = (DPJArrayBoolean<this:[0]>) A.subarray(0, idxs[0]);
	    for (i = 1; i < idxs.length; ++i) {
		len = idxs[i] - idxs[i-1];
		if (len < 0) 
		    throw new ArrayIndexOutOfBoundsException();
	    	final int j = i;
		segs[j] = (DPJArrayBoolean<this:[j]>) A.subarray(idxs[j-1], len);	    
	    }
            i = idxs[idxs.length - 1];
            len = A.length - i;
	    final int length = idxs.length;
            segs[length] = (DPJArrayBoolean<this:[length]>) A.subarray(i, len);
	}
    }
     ...
     */
    public DPJArrayBoolean<this : [idx] : *> get(final int idx) reads this:[idx] {
	...
	if (segs != null)
	    ...
	    return (DPJArrayBoolean<this:[idx]:*>) A.subarray(start, segLength);
        }
    }
